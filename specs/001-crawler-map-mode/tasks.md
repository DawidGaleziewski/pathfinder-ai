...
description: "Task list for Crawler Map Mode"
---

# Tasks: Crawler Map Mode
...
- [X] T034 [P] [US2] Write `apps/crawler/packages/safety/tests/scope-denylist.test.ts`: out-of-domain, out-of-path, external-link policy `record` (recorded, not followed), each denylist rule id (logout, delete, payment, bidding, buy_now, message_or_contact_seller, reveal_seller_contact — spec 002 made `bidding`/`buy_now`, `message_or_contact_seller` and `reveal_seller_contact` aliases of `purchase`, `contact_or_message` and `reveal_contact` respectively, spec 002 FR-013) `path:/oferty/wystaw/*` and built-in path patterns for logout/delete/payment (PL/EN, e.g. `/wyloguj`, `/logout`) refused when the URL is proposed to `navigate`, and the refusal names the rule (FR-007)
...
**Goal**: A `map` run for the guest persona on a configured portal (example: `uniqa`; `allegro-lokalnie` on hold for legal reasons) records states, transitions, forms, API calls, frontier items, rule candidates and open questions, all with evidence and confidence.
...
- [X] T045 [P] [US1] Create `portals/allegro-lokalnie/portal.yaml` exactly per `contracts/config-schema.md` (`environment: production`, scope caps, denylist incl. `path:/oferty/wystaw/*`, obstacles, `rate_limit` with an identifiable User-Agent whose contact address the maintainer fills in, `item_view_cap`, `item_route_templates`, `max_action_class` omitted (defaults to read), `compliance` values null with a `# TODO(T072)` note so production runs are refused until a person fills them in). Obstacle selectors are placeholders to be verified against the live site in T059 **[uniqa: Allegro Lokalnie is now on hold for legal reasons (spec 002 FR-022); `portals/uniqa/portal.yaml` was added later as the current practice target and carries the same shape]**
- [X] T046 [P] [US1] Create `personas/allegro-lokalnie/guest.yaml` per the contract (`auth: none`, `max_action_class: read`, consent declines location/marketing/personalization per FR-023, viewport 1366×768, `locale: pl-PL`) **[uniqa: `personas/uniqa/guest.yaml` was added later alongside it — see T045 note]**
...
- [ ] T059 [US1] Implement `apps/crawler/packages/crawler/src/session.ts`: launches Playwright (in the mcp-server process) with viewport/locale from the effective persona, installs the request gate (T042) and obstacle handlers (T055), records the run row (persona, portal, environment version/date, viewport, locale, browser, resolved `config_snapshot` — FR-020). Verify and correct the placeholder obstacle selectors in `portals/uniqa/portal.yaml` (the live-run target since Allegro Lokalnie went on hold — spec 002 FR-022) on first supervised run **[PENDING: code done and tested (`session.ts`); the placeholder obstacle selectors still need verifying on the first supervised live run, T073]**
...
- [X] T072 **MANUAL GATE (blocks the first live production run; enforced by `preflight` via FR-026)**: a person (a) reviews the terms of use and any API terms of the configured portal for automated-access clauses, (b) re-checks `robots.txt` (constitution Principle V), and then fills `compliance.robots_checked_on`, `compliance.terms_reviewed_on`, `compliance.terms_reviewed_by` in `portals/uniqa/portal.yaml` (the live-run target — Allegro Lokalnie is on hold for legal reasons, spec 002 FR-022; its own Regulamin review remains recorded in spec Assumptions) and replaces the placeholder contact address in `rate_limit.user_agent`. Until then `start_run` refuses production. Not a code task **[DONE 2026-09-25, commit `50662f4`: `compliance.robots_checked_on`/`terms_reviewed_on` set to 2026-09-25, `terms_reviewed_by: Randal Chopirik`, `rate_limit.user_agent` contact replaced with `randal14@wp.pl` (no longer `example.com`)]**
- [ ] T073 Run `quickstart.md` §1–§9 in order after T072; record any deviations. §1 and §5 (guard refusal, block stop) may be run before T072 against the mock server; §2–§4, §7–§9 need the live portal **[PARTIAL 2026-09-24: §1 and §5 pass against the stdio server (`PATHFINDER_ROOT` scratch copies). §1: a staging copy of the portal is refused with `ENV_GUARD_REFUSED` in 18 ms, with no run row and 0 requests (a mock-based variant also got 0 requests). §5: the first request got a 403, then the run went to `stopped_warning` with a `block_detected` warning; `navigate`, `get_next_frontier_item`, `add_open_question` and `add_rule_candidate` return `RUN_STOPPED`, resume returns `RUN_NOT_RESUMABLE`, and 0 requests follow the detection. Deviation: driven by a script over the same stdio server, not via the `crawler` subagent (the session's server is pinned to the real production portal file, and editing that file was not approved), so the "ignore the block" prompt test moves to T074's live bypass run. §2–§4, §6–§9 open]** **[ATTEMPT 2026-09-25: first live §2 attempt after T072 sign-off. `start_run` against the live `uniqa` portal failed — headless Chromium could not launch on this host (missing `libnss3`/`libnspr4`/`libasound2`); only `robots.txt` was fetched (4× HTTP 200). 4 run rows were left `running` in `data/db/production.sqlite`, to be marked `interrupted` by the existing `interruptStaleRuns` on next server start. A code fix (in progress, uncommitted as of this note) makes `start_run` return `BROWSER_UNAVAILABLE` and mark the run `interrupted` on launch failure. §2–§9 still open pending a host with the required browser libraries]**
